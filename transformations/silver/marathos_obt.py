...
def clean_marathos():
    ...
    df = (
        df.where(col("distance_type") != "days")
    ...
    df = df.withColumn(
        "performance_seconds",
        when(
            col("distance_type").isin("km", "miles"),
            regexp_extract(
                col("athlete_performance"), r"(\d+):(\d{2}):(\d{2})", 1
            ).cast("int")
            * 3600
            + regexp_extract(
                col("athlete_performance"), r"(\d+):(\d{2}):(\d{2})", 2
            ).cast("int")
            * 60
            + regexp_extract(
                col("athlete_performance"), r"(\d+):(\d{2}):(\d{2})", 3
            ).cast("int"),
        ).otherwise(None),
    )
    ...
    df = df.withColumn(
        ...
        when(
            ...
            when(
                ...
                round(
                    regexp_extract(
                        col("event_distance_or_length"), r"(\d+\.?\d*)", 1
                    ).cast("double")
        ...
        .when(
            col("distance_type") == "time",
            when(
                regexp_extract(col("event_distance_or_length"), r"(\d+\.?\d*)", 1).cast(
                    "double"
                )
                > 0,
                round(
    ...
    # Step 11 - event distance and event duration
    # Extract distance in km
    df = df.withColumn(
        "event_distance_km",
        when(
            col("distance_type") == "km",
            regexp_extract(col("event_distance_or_length"), r"(\d+\.?\d*)", 1).cast(
                "double"
            ),
        )
    ...
    df = df.withColumn(
        ...
        when(
            ...
            when(
                col("event_distance_or_length").rlike(r"^\d+:\d{2}h?$"),
                round(
                    regexp_extract(col("event_distance_or_length"), r"^(\d+):", 1).cast(
                        "double"
                    )
                    + regexp_extract(
                        col("event_distance_or_length"), r":(\d{2})h?$", 1
                    ).cast("double")
                    / 60,
                    2,
                ),
    ...
    # Step 12 - Giving unique ids to events and athletes(some athlete ids are placeholders)
    df = df.withColumn(
        "event_id",
        sha2(
